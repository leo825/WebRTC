var WebSocketServer = require('ws').Server;
var UUID = require('node-uuid');
var events = require('events');
var util = require('util');
var errorCb = function(rtc) {
	return function(error) {
		if (error) {
			rtc.emit("error", error);
		}
	};
};

function SkyRTC() {
	this.sockets = [];
	this.rooms = {};

	//获取指定房间成员，并以json数组返回
	function getRoomUsersAsJson(room){
		var curRoom = room;
		var curSocket;
		var jsonResult = [];
		for(i=0; i<curRoom.length; i++){
			curSocket = curRoom[i];
			var item = {};
			item['socketId'] = curSocket.id;
			item['userId'] = curSocket.userId;
			item['userName'] = curSocket.userName;
			jsonResult.push(item);
		}
		return jsonResult;
	}

	this.on('__join', function(data, socket) {
		console.log(this.sockets.length);
		var ids = [],
			i, m,
			room = data.room || "__default",
			curSocket,
			curRoom;

		curRoom = this.rooms[room] = this.rooms[room] || [];
		console.log("现在的roomNum==="+data.room+", 现在的curRoom长度是 "+curRoom.length);

		for (i = 0, m = curRoom.length; i < m; i++) {
			curSocket = curRoom[i];
			if (curSocket.id === socket.id) {
				continue;
			}
			ids.push(curSocket.id);
			curSocket.send(JSON.stringify({
				"eventName": "_new_peer",
				"data": {
					"socketId": socket.id,
					"userId": data.userId,
					"userName": data.userName
				}
			}), errorCb);
		}

		curRoom.push(socket);
		socket.room = room;
		socket.userId = data.userId;
		socket.userName = data.userName;

		var users = getRoomUsersAsJson(curRoom) || [];
		socket.send(JSON.stringify({
			"eventName": "_peers",
			"data": {
				"connections": ids,
				"you": socket.id,
				"users": users
			}
		}), errorCb);

		this.emit('new_peer', socket, room);
	});
	
	this.on('__ice_candidate', function(data, socket) {
		var soc = this.getSocket(data.socketId);

		if (soc) {
			soc.send(JSON.stringify({
				"eventName": "_ice_candidate",
				"data": {
					"label": data.label,
					"candidate": data.candidate,
					"socketId": socket.id
				}
			}), errorCb);

			this.emit('ice_candidate', socket, data);
		}
	});

	this.on('__offer', function(data, socket) {
		var soc = this.getSocket(data.socketId);

		if (soc) {
			soc.send(JSON.stringify({
				"eventName": "_offer",
				"data": {
					"sdp": data.sdp,
					"socketId": socket.id
				}
			}), errorCb);
		}
		this.emit('offer', socket, data);
	});

	this.on('__answer', function(data, socket) {
		var soc = this.getSocket(data.socketId);
		if (soc) {
			soc.send(JSON.stringify({
				"eventName": "_answer",
				"data": {
					"sdp": data.sdp,
					"socketId": socket.id
				}
			}), errorCb);
			this.emit('answer', socket, data);
		}
	});
	
	//从服务器中移除此room,把客户端和服务器连接的socket断掉
	this.on('__remove_room',function(roomNum){
	    var that = this;
		var i,m,curRoom,curSocket;
		console.log('此时要__remove_room是 '+roomNum);
		curRoom = this.rooms[roomNum]=this.rooms[roomNum] || [];
		m = curRoom.length;
		if(roomNum && curRoom){
			curRoom = that.rooms[roomNum];
			for (i = curRoom.length; i--;) {
				curRoom[i].send(JSON.stringify({
					"eventName": "_remove_room_peer",
					"data": {
						"socketId": curRoom[i].id
					}
				}), errorCb);
			}
		}
		that.removeRoomPeers(roomNum);

		this.rooms[roomNum].splice(0,m-1);
		if (this.rooms[roomNum].length === 0) {
			delete this.rooms[roomNum];
		}
		that.emit('remove_room',roomNum);
	});
}

util.inherits(SkyRTC, events.EventEmitter);

/**
 * 向指定会议室内的所有人发送api命令，如分屏命令等
 * @param roomId
 * @param apiName
 * @param apiData
 */
SkyRTC.prototype.sendApiToRoom = function (roomId, apiName, apiData) {
    console.log("开始向房间[" + roomId + "]执行命令[" + apiName + ":" + apiData + "]");
    var room = this.rooms[roomId];
    if(room){
        for (i = 0, m = room.length; i < m; i++) {
            room[i].send(JSON.stringify({
                "api": apiName,
                "data": apiData
            }), errorCb);
        }
    }else{
        console.log("发送api失败，因会议室" + roomId + "中无人");
    }
}

/**
 * 向指定的人发送消息
 * @reqUser 请求发送消息的id
 * @resToUser 接收消息的id
 * @reqMsg 发送消息内容
 *
 * */
SkyRTC.prototype.sendMessageToUser=function(reqUser,resToUser,reqMsg){
    console.log("服务端接到["+reqUser+"]的请求，开始向["+resToUser+"]发起消息，消息内容["+reqMsg+"]");
    var result,curRoom, i, m,curSocket;
    var rooms = this.rooms;
    result = false;
    for(var room in rooms){
        curRoom = rooms[room] = rooms[room] || [];
        for (i = 0, m = curRoom.length; i < m; i++) {
            curSocket = curRoom[i];
            if (curSocket.userId === resToUser) {
                result = true;
                curSocket.send(JSON.stringify({
                    "eventName": "message",
                    "data": {
                        "reqUser": reqUser,
                        "resToUser": resToUser,
                        "reqMsg": reqMsg
                    }
                }), errorCb);
                return result;
            }else{
                continue;
            }
        }
    }
    return result;
}


SkyRTC.prototype.addSocket = function(socket) {
	this.sockets.push(socket);
};

//移除此房间所有成员
SkyRTC.prototype.removeRoomPeers = function(room){
	var that = this;
	var i,j,m,curRoom,curSocket;
	curRoom = this.rooms[room]=this.rooms[room] || [];
	m = curRoom.length;

	if(room && curRoom){
		for(i = 0; i < m; i++){
			curSocket = curRoom[i];
			if(curSocket){
				j = this.sockets.indexOf(curSocket);
				this.sockets.splice(j,1);
				that.emit('remove_peer', curSocket.id, that);
				console.log("removeRoomPeers被调用, "+curSocket.id+" 在this.sockets中已经被移除了");
			}
		}
	}
}

SkyRTC.prototype.removeSocket = function(socket) {
	var i = this.sockets.indexOf(socket),
		room = socket.room;
	this.sockets.splice(i, 1);
	if (room) {
		i = this.rooms[room].indexOf(socket);
		this.rooms[room].splice(i, 1);
		if (this.rooms[room].length === 0) {
			delete this.rooms[room];
		}
	}
};

SkyRTC.prototype.broadcast = function(data, errorCb) {
	var i;
	for (i = this.sockets.length; i--;) {
		this.sockets[i].send(data, errorCb);
	}
};

SkyRTC.prototype.broadcastInRoom = function(room, data, errorCb) {
	var curRoom = this.rooms[room],
		i;
	if (curRoom) {
		for (i = curRoom.length; i--;) {
			curRoom[i].send(data, errorCb);
		}
	}
};

SkyRTC.prototype.getRooms = function() {
	var rooms = [],
		room;
	for (room in this.rooms) {
		rooms.push(room);
	}
	return rooms;
};

SkyRTC.prototype.getSocket = function(id) {
	var i,
		curSocket;
	if (!this.sockets) {
		return;
	}
	for (i = this.sockets.length; i--;) {
		curSocket = this.sockets[i];
		if (id === curSocket.id) {
			return curSocket;
		}
	}
	return;
};

//移除socket连接
SkyRTC.prototype.removeRoom = function(socket){
	socket.on('close', function() {
		var i, m,
			room = socket.room,
			curRoom;
		if (room) {
			curRoom = that.rooms[room];
			for (i = curRoom.length; i--;) {
				if (curRoom[i].id === socket.id) {
					continue;
				}
				curRoom[i].send(JSON.stringify({
					"eventName": "_remove_peer",
					"data": {
						"socketId": socket.id
					}
				}), errorCb);
			}
		}

		that.removeSocket(socket);
		that.emit('remove_peer', socket.id, that);
		console.log("removeRoom 方法被调用, "+socket.id+" 被移除了");
	});
	
}



SkyRTC.prototype.init = function(socket) {
	var that = this;
	socket.id = UUID.v4();
	that.addSocket(socket);
	//为新连接绑定事件处理器
	socket.on('message', function(data) {
		var json = JSON.parse(data);
		if (json.eventName) {
			that.emit(json.eventName, json.data, socket);
		} else {
			that.emit("socket_message", socket, data);
		}
	});
	//连接关闭后从SkyRTC实例中移除连接，并通知其他连接
	socket.on('close', function() {
		var i, m,
			room = socket.room,
			curRoom;
		if (room) {
			curRoom = that.rooms[room];
			for (i = curRoom.length; i--;) {
				if (curRoom[i].id === socket.id) {
					continue;
				}
				curRoom[i].send(JSON.stringify({
					"eventName": "_remove_peer",
					"data": {
						"socketId": socket.id
					}
				}), errorCb);
			}
		}
		that.removeSocket(socket);
		that.emit('remove_peer', socket, that);
		console.log("socket的close事件被调用, "+socket.id+" 被移除了.");
	});
	that.emit('new_connect', socket);
};

module.exports.listen = function(server) {
	var SkyRTCServer;
	if (typeof server === 'number') {
		SkyRTCServer = new WebSocketServer({
			port: server
		});
	} else {
		SkyRTCServer = new WebSocketServer({
			server: server
		});
	}

	SkyRTCServer.rtc = new SkyRTC();
	errorCb = errorCb(SkyRTCServer.rtc);
	SkyRTCServer.on('connection', function(socket) {
		this.rtc.init(socket);
	});

	return SkyRTCServer;
};